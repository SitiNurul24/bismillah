
    <?php
    
    

    //import database
    include("../koneksi.php");



    if($_POST){
        //print_r($_POST);
        $result= $database->query("select * from webpengguna");
        $name=$_POST['name'];
        $nic=$_POST['nic'];
        $oldemail=$_POST["oldemail"];
        $alamat=$_POST['alamat'];
        $email=$_POST['email'];
        $tele=$_POST['Tele'];
        $password=$_POST['password'];
        $cpassword=$_POST['cpassword'];
        $id=$_POST['id00'];
        
        if ($password==$cpassword){
            $error='3';
            $aab="select pasien.pid from pasien inner join webpengguna on pasien.pemail=webpengguna.email where webpengguna.email='$email';";
            $result= $database->query($aab);
            //$resultqq= $database->query("select * from doctor where docid='$id';");
            if($result->num_rows==1){
                $id2=$result->fetch_assoc()["pid"];
            }else{
                $id2=$id;
            }
            

            if($id2!=$id){
                $error='1';
                //$resultqq1= $database->query("select * from doctor where docemail='$email';");
                //$did= $resultqq1->fetch_assoc()["docid"];
                //if($resultqq1->num_rows==1){
                    
            }else{

                //$sql1="insert into doctor(docemail,docname,docpassword,docnic,doctel,specialties) values('$email','$name','$password','$nic','$tele',$spec);";
                $sql1="update pasien set pemail='$email',pnama='$nama',pkatasandi='$password',pnik='$nik',ptel='$tele',palamat='$alamat' where pid=$id ;";
                $database->query($sql1);
                echo $sql1;
                $sql1="update webpengguna set email='$email' where email='$oldemail' ;";
                $database->query($sql1);
                echo $sql1;
                
                $error= '4';
                
            }
            
        }else{
            $error='2';
        }
    
    
        
        
    }else{
        //header('location: signup.php');
        $error='3';
    }
    

    header("location: pengaturan.php?action=edit&error=".$error."&id=".$id);
    ?>
    
   

</body>
</html>